...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < traderAddresses.length; i++) {
            ...
            let result: TraderAuditResult | undefined = undefined;
            if (trader) {
                result = await simulateTrader(trader, capitalPerTrader);
            }
            if (result) {
                individualResults.push(result);
            }

            if (!result?.error && (result?.copiedTrades ?? 0) > 0) {
                const roiColor = (result?.roi ?? 0) >= 0 ? colors.green : colors.red;
                console.log(
                    `  ${roiColor((result?.roi ?? 0) >= 0 ? '✓' : '✗')} ROI: ${(result?.roi ?? 0).toFixed(2)}% | P&L: $${result?.totalPnl?.toFixed(2) ?? 0} | Trades: ${result?.copiedTrades ?? 0}`
                );
            } else {
        ...
        const worstTrader = validResults.reduce(
            (worst, current) => (current.roi < (worst?.roi ?? 0) ? current : worst),
            validResults[0]
        );
        ...
        const report: AuditReport = {
            ...
            analysis: {
                ...
                bestTrader: bestTrader?.shortAddress || 'N/A',
                worstTrader: worstTrader?.shortAddress || 'N/A',
                avgWinRate,
