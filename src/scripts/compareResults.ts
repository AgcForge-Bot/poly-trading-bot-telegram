...
function printBestResults(results: SimulationResult[], limit: number = 5) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        console.log(colors.bold(`${medal} ${result?.name ?? 'N/A'}`));
        console.log(colors.gray(`   Trader: ${result?.traderAddress?.slice(0, 10) ?? 'N/A'}...`));
        console.log(
            `   ROI: ${(result?.roi ?? 0) >= 0 ? colors.green(`+${(result?.roi ?? 0).toFixed(2)}%`) : colors.red(`${(result?.roi ?? 0).toFixed(2)}%`)}`
        );
        console.log(
            `   P&L: ${(result?.totalPnl ?? 0) >= 0 ? colors.green(`+$${(result?.totalPnl ?? 0).toFixed(2)}`) : colors.red(`-$${Math.abs(result?.totalPnl ?? 0).toFixed(2)}`)}`
        );
        console.log(`   Trades: ${(result?.copiedTrades ?? 0)} copied, ${(result?.skippedTrades ?? 0)} skipped`);
        console.log(
            `   Capital: $${(result?.startingCapital ?? 0).toFixed(2)} → $${(result?.currentCapital ?? 0).toFixed(2)}`
        );
        console.log();
    }
}
...
function printWorstResults(results: SimulationResult[], limit: number = 3) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        console.log(colors.bold(`${i + 1}. ${result?.name ?? 'N/A'}`));
        console.log(colors.gray(`   Trader: ${result?.traderAddress?.slice(0, 10) ?? 'N/A'}...`));
        console.log(
            `   ROI: ${(result?.roi ?? 0) >= 0 ? colors.green(`+${(result?.roi ?? 0).toFixed(2)}%`) : colors.red(`${(result?.roi ?? 0).toFixed(2)}%`)}`
        );
