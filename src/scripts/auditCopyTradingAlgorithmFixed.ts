    ...
    try {
        ...
        for (const trade of trades) {
            ...
            if (trade.side === 'BUY') {
                ...
                (pos.trades as unknown as TradeSimulatedPosition[]).push({
                    timestamp: trade.timestamp,
                    side: 'BUY',
                    price: trade.price,
                    size: sharesReceived,
                    usdcSize: orderSize,
                    traderSize: COPY_PERCENTAGE,
                    yourSize: orderSize,
                });
...
function printAuditReport(report: AuditReport) {
    ...
    console.log(
        `  Copy Percentage: ${colors.yellow((report.config as AuditReportConfig).copyPercentage + '%')} of trader's order size`
    );
    console.log(
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < traderAddresses.length; i++) {
            ...
            const result = await simulateTrader(trader as string, capitalPerTrader);
        ...
        const bestTrader = validResults.reduce(
            (best, current) => (best && current.roi > best.roi ? current : best),
            validResults[0]
        );

        const worstTrader = validResults.reduce(
            (worst, current) => (worst && current.roi < worst.roi ? current : worst),
            validResults[0]
        );
