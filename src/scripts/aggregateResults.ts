...
async function aggregateResults() {
    ...
    if (strategies.length > 0) {
        ...
        console.log(`  ID: ${colors.yellow(best?.strategyId || 'N/A')}`);
        console.log(`  ROI: ${colors.green('+' + best?.bestROI.toFixed(2) + '%' || 'N/A')}`);
        console.log(`  Win Rate: ${colors.yellow(best?.bestWinRate.toFixed(1) + '%' || 'N/A')}`);
        console.log(`  P&L: ${colors.green('+$' + best?.bestPnL.toFixed(2) || 'N/A')}`);
    }
